...
// 	jr2 := job.NewRequest(j2.GetID(), exec4)
// 	callbackJR := job.NewRequest(callback.GetID(), exec5)
// 	c, err := chord.NewChord([]job.Request{*jr, *jr2}, *callbackJR, bc, pq, cache.NewJobCacheNoWatch(bc))
// 	assert.NoError(t, err)
// 	c.Dispatch()
