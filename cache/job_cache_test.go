...
func TestJobCache(t *testing.T) {
	...
	assert.NoError(t, err)
	blk1 := core.NewBlock(*tree1, latest_block.GetHeader().GetHash(), next_height, 10, "74657374")
	bc.AddBlock(blk1)
	latest_block, err = bc.GetLatestBlock()
	assert.NoError(t, err)
	next_height, err = bc.GetNextHeight()
	assert.NoError(t, err)
	blk2 := core.NewBlock(*tree2, latest_block.GetHeader().GetHash(), next_height, 10, "74657374")
	bc.AddBlock(blk2)
	latest_block, err = bc.GetLatestBlock()
	assert.NoError(t, err)
	next_height, err = bc.GetNextHeight()
	assert.NoError(t, err)
	blk3 := core.NewBlock(*tree3, latest_block.GetHeader().GetHash(), next_height, 10, "74657374")
	bc.AddBlock(blk3)
