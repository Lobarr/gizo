...
func (d Dispatcher) BlockReqMultiple(ctx context.Context, args wamp.List, kwargs, details wamp.Dict) *client.InvokeResult {
	var blocks []*core.Block
	for _, hash := range args {
		blockinfo, _ := d.GetBC().GetBlockInfo(hash.(string))
		blocks = append(blocks, blockinfo.GetBlock())
	}
	return &client.InvokeResult{Args: wamp.List{blocks}}
}
